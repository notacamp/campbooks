...
- **Compose from intent (bold layout)**: on the bold Desk (`/email_messages/new`), `Emails::IntentPrefill` infers To/Subject from `?intent=`/`?to=` (shown as "· inferred" chips), the `Campbooks::Compose::IntentInput` drives the existing compose-chat to fill the editor (Scout's-draft mark clears on edit), and `Ai::DraftRewriter` powers the "Shorter"/"Warmer" footer via `POST /email_messages/rewrite_draft`; the classic composer is unchanged (`bold_layout?`-guarded).
...
- `ENABLE_BOLD_LAYOUT` — opt-in gate for the rethought "bold" layout: the Now page + the Now/People/Paper/Money/Time navigation + the global **Scout overlay** (the docked bar and `⌘K` open Scout + the command palette in place on every bold surface). Off by default; users then choose Bold or Classic in Settings › Account.
- `ENABLE_WORKFLOWS`, `ENABLE_EMAIL_BOARD`, `ENABLE_MICROSOFT`, `ENABLE_BOLD_LAYOUT` — **production-readiness feature gates** (`Features`, `app/models/features.rb`): features that are built but not yet production-ready ship OFF by default in every build; set to `1` to opt in. Orthogonal to the per-workspace `Entitlements` billing gates. Gate, respectively, the Workflow engine, the inbox "Board" (status kanban) layout, all Microsoft 365 surfaces, and the bold layout (the Now page + the Now/People/Paper/Money/Time navigation — users then choose Bold or Classic in Settings › Account). Exception: `ENABLE_IMAP` is an **opt-OUT** gate — IMAP mailbox connect ships ON; set `ENABLE_IMAP=0` to hide it.
- `ENABLE_WORKFLOWS`, `ENABLE_EMAIL_BOARD`, `ENABLE_MICROSOFT`, `ENABLE_BOLD_LAYOUT` — **production-readiness feature gates** (`Features`, `app/models/features.rb`): features that are built but not yet production-ready ship OFF by default in every build; set to `1` to opt in. Orthogonal to the per-workspace `Entitlements` billing gates. Gate, respectively, the Workflow engine, the inbox "Board" (status kanban) layout, all Microsoft 365 surfaces, and the rethought bold layout — the five-place Now/People/Paper/Money/Time navigation, the Now decision deck, and **Paper** (`/paper`, `PaperController`): every document rendered as a row of facts (kind, amount, the date that matters) with a derived status from `Documents::Status`/`Facts`/`Summary`, plus per-user Bold/Classic in Settings › Account. Exception: `ENABLE_IMAP` is an **opt-OUT** gate — IMAP mailbox connect ships ON; set `ENABLE_IMAP=0` to hide it.
- `ENABLE_BOLD_LAYOUT` — opt-in gate (`Features.bold_layout?`) for the rethought **bold layout**: the Now page and the Now/People/Paper/Money/Time navigation; each user then picks Bold or Classic in Settings › Account. In the **People** place (`/people`) every sender is a person or a **service** (machine/bulk mail — newsletters, receipts, notifications), classified from its recent mail by `Contacts::SenderKind` and correctable by hand; a person opens as their email threads newest-first, each thread opening on its newest message with older ones folded beneath, every open message carrying Reply / Reply all / Forward through the Compose Dock, and an organization shows its people and services side by side. **Streams are the workspace's inbox groups** — `Emails::TagGroups` over `InboxGroupRule` — surfaced as a services view; rows act through `EmailActions` (`People::ActionsController`: Reply, Done, Snooze, Star, Archive with undo) and update live on the `people_<user_id>` stream; every message carries a `channel` (email today) so a second channel is additive. The People list is backed by the **`people_standings` materialized table** (`People::Standings.refresh!`, `People::StandingsRefreshJob`) — on first visit the directory computes inline; subsequent visits read the table and enqueue a background refresh when rows are older than 10 minutes; the table is refreshed automatically after every `Feed::RefreshJob` and `Contacts::SenderKindBackfillJob`. **Standings are projected from the feed** (`People::Attention`) into verb lanes (Reply, Decide, Pay, Chase, Nudge) — counterparts with an active feed item appear at the top (five per lane, the rest folded); below them **Latest** lists everyone by newest message, newest first (the inbox proper — lane people appear there too), infinite-scrolled 30 per page. The page opens on the latest received message; opening a person marks their newest thread read through `Emails::MarkThreadRead` (shared with the classic thread view: local + provider + live inbox) and loads only that thread's messages — older threads and folded message bodies are lazy turbo-frames served by `People::ThreadsController` / `People::MessagesController`.
