...
- `ENABLE_WORKFLOWS`, `ENABLE_EMAIL_BOARD`, `ENABLE_MICROSOFT` — **production-readiness feature gates** (`Features`, `app/models/features.rb`): features that are built but not yet production-ready ship OFF by default in every build; set to `1` to opt in. Orthogonal to the per-workspace `Entitlements` billing gates. Gate, respectively, the Workflow engine, the inbox "Board" (status kanban) layout, and all Microsoft 365 surfaces.
