...
Not A Camp's AI-native email client — an inbox that sorts itself, for young professionals and small-business owners drowning in email and paperwork. Rails 8.1, PostgreSQL, Tailwind CSS 4, Hotwire, Solid Queue, Phlex components. The app is the five-place layout — Now, People (the inbox), Paper, Money, Time — with the global Scout overlay accessible from every surface via the docked bar and ⌘K.
...
- **IMAP (any provider)**: `EmailAccount` provider `imap` authenticates with a password (`imap_password`, encrypted; `refresh_token` is nullable for it) against user-supplied IMAP/SMTP hosts. Connect/re-auth form at `/imap_accounts/new|edit` (`ImapAccountsController` live-verifies credentials via `Imap::MailClient#verify!` before saving; `Imap::HostGuard` blocks private/metadata hosts on cloud, allows them when `SELF_HOSTED`/development). Sync = `Emails::SyncStrategies::Imap` (per-folder `uidvalidity:uidnext` cursor in `email_folders.delta_token`; minute poll skips folders whose UIDNEXT is unchanged; periodic sweep re-walks a 90-day-clamped window; first walk honors the connect-time `backfill_since` choice). Message identity = normalized RFC822 `Message-ID` (moves keep it; cross-folder dupes collapse). Outbound = SMTP + APPEND to Sent; drafts live in the Drafts folder. No labels/calendar for IMAP. Gated by `Features.imap?` — **on by default**, `ENABLE_IMAP=0` hides connect surfaces.
- **Views**: `/email_accounts` (connect/status), `/email_scans` (audit trail), `/email_messages` (all ingested emails)
- **Compose from intent**: on the Desk (`/email_messages/new`), `Emails::IntentPrefill` infers To/Subject from `?intent=`/`?to=` (shown as "· inferred" chips), the `Campbooks::Compose::IntentInput` drives the existing compose-chat to fill the editor (Scout's-draft mark clears on edit), and `Ai::DraftRewriter` powers the "Shorter"/"Warmer" footer via `POST /email_messages/rewrite_draft`.

## People (the inbox)

Every sender is a person or a **service** (machine/bulk mail — newsletters, receipts, notifications), classified from its recent mail by `Contacts::SenderKind` and correctable by hand; a person opens as their email threads newest-first, each thread opening on its newest message with older ones folded beneath, every open message carrying Reply / Reply all / Forward through the Compose Dock, and an organization shows its people and services side by side. **Streams are the workspace's inbox groups** — `Emails::TagGroups` over `InboxGroupRule` — surfaced as a services view; rows act through `EmailActions` (`People::ActionsController`: Reply, Done, Snooze, Star, Archive with undo) and update live on the `people_<user_id>` stream; every message carries a `channel` (email today) so a second channel is additive. The People list is backed by the **`people_standings` materialized table** (`People::Standings.refresh!`, `People::StandingsRefreshJob`) — on first visit the directory computes inline; subsequent visits read the table and enqueue a background refresh when rows are older than 10 minutes; the table is refreshed automatically after every `Feed::RefreshJob` and `Contacts::SenderKindBackfillJob`. **Standings are projected from the feed and from accepted asks** (`People::Attention`) into verb lanes (Reply, Decide, **Do**, Pay, Chase, Nudge) — counterparts with an active feed item or an open accepted ask appear at the top (five per lane, the rest folded); the Do lane is fed by accepted open `Task` records whose source email has a person contact — `People::Attention#build_ask_items` (gated by `Features.tasks?`); the `data["ask"]` jsonb carries `id/due_on/held_at` so the `PeopleStanding` row can re-render chips without re-querying; below them **Latest** lists everyone by newest message, newest first (the inbox proper — lane people appear there too, carrying their verb), infinite-scrolled 30 per page. **Scout's line on a row and the "where things stand" note** (`Campbooks::People::StandNote`) say what is open with that person, never the contact bio: a standing carries a `detail` + `detail_kind` (`ask_ai` from the analyzer's `email_messages.ai_ask`, `ask_quote` = the sender's own sentence via `Emails::AskExtractor` / `People::Ask`, `reason`/`silence` for a follow-up, `prompt` for a Decide row, `money` for a late invoice with the amounts in `data["money"]`, `you_wrote_last` when you hold the last word) and `People::StandCopy` composes the row line and the note in the user's locale at render time; the note's chips act through the existing tools (`draft_reply`/`draft_follow_up`), the People actions (`done`/`snooze`/`paid`) and the `scout:ask` window event (Ask Scout). `EmailProcessJob` enqueues `EmailAnalysisJob` (Scout's full read) for inbound human mail, so the Reply/Decide lanes and the Now feed's email cards fill in. The page opens on the latest received message; opening a person marks their newest thread read through `Emails::MarkThreadRead` (shared with the thread view: local + provider + live inbox) and loads only that thread's messages — older threads and folded message bodies are lazy turbo-frames served by `People::ThreadsController` / `People::MessagesController`. A message permalink (`/email_messages/:id`) for a message whose contact has a person in the workspace redirects to `/people/:person?thread=:thread_id` so the conversation stays in context.
...
- **Time surface** (`/time`, `TimeController`): reuses the calendar's loading (`Calendars::PageData`, now shared with `CalendarController`) and merges events + pending reminders (deadlines) + live **asks** (the `Task` record) + Scout-proposed `FocusBlock`s into one agenda (`Time::Agenda` → `Time::AgendaItem`) with a pure Scout day note (`Time::DayNote`), a working-hours slot finder (`Time::SlotFinder`) and proposer (`Time::FocusProposer`); a block's Keep becomes a real event (`Time::FocusKeeper`, reusing `Reminders::Confirm`'s calendar pick via `Calendars::WritableTarget`) and reminders stop being a tab (`Campbooks::Calendar::ViewTabs reminders: false`). Adds a device-captured **`users.time_zone`** (the `local-greeting` controller PATCHes `/account/time_zone` once when it's blank) → `User#effective_time_zone` (user zone → primary calendar zone → UTC) for day bucketing + slot finding.
- **Asks** (the rethought Tasks module): a task is an *ask* with no page of its own — a card on Now while it needs a decision (its email card's chip row, or a decision card for the undated / due / overdue), and a row on Time once dated (undated ones under "No date yet"). The only question is *when*: **Hold time** (`Time::FocusHolder` stakes + keeps a `FocusBlock` in Scout's free slot), **Set a date**, or **Not now** (`tasks.snoozed_until` — a week's snooze); plus Done/dismiss. All routed through `AsksController` (`/asks/:id/{hold,schedule,snooze,done,dismiss}`) and the feed act tools (`hold_task`/`schedule_task`/`snooze_task`), each re-rendering the Time agenda / feed card with an Undo. The board, list, Skim page, task pages and task discussions are retired; `/tasks` + `/tasks/:id` **redirect to Time** (`TasksController`). The `Task` model, API and MCP tools are unchanged. Asks are **core**: on by default (`Features.tasks?` is opt-OUT — `ENABLE_TASKS=0` hides them) and no longer a paid feature (the `:tasks` entitlement was removed). One AI read per email — `Ai::EmailAnalyzer` returns the structured `asks` and stages them via `Tasks::Builder` in the same call; the standalone `Tasks::EmailExtractionJob` / `Ai::TaskExtractor` survive only for the backfill rake and document extraction.
  - **Whose ask it is** on the personal surfaces (Now / Time / People) is `Task.for_user` — assigned to you, or unassigned and from a Document, a manual source, or an `EmailMessage` you can read — while `accessible_to` (workspace-wide) stays for the API and MCP.
  - **Hand-off** is the one team action: `AsksController#hand_off` → `Asks::HandOff` reassigns the ask to one teammate (accepting it), publishes `task.handed_off` and raises an action-required notice on their Now (auto-resolved when the ask is done/cancelled, or taken back via `#take_back`); the assigner's Time keeps a muted "with <name>" row (Take it back / Done).
  - **Scout answers "what do I owe people?"** via the `query_asks` read tool (`Tools::QueryAsks` over `for_user`), rendered as ask cards under the reply (`Campbooks::Scout::AskCards`); the public API adds `/api/v1/asks` as an alias of `/api/v1/tasks`.

## Workflow engine

> ⚠️ **Disabled by default (not production-ready).** The whole engine — builder UI, nav/Cmd+K entries, controllers, the public webhook ingress, the API, and the automatic email/event triggers — is gated off behind `Features.workflows?` (`ENABLE_WORKFLOWS=1`). When off the UI/API 404 and no trigger fires (the enqueue sites and the jobs both bail). The architecture below is unchanged — it's just inert until the flag is set.
...
- `MICROSOFT_CLIENT_ID`, `MICROSOFT_CLIENT_SECRET` — Microsoft 365 "Sign in with Microsoft" + mailbox connect. Register an app in Entra (`https://entra.microsoft.com` → App registrations), supported account types "any organizational directory (multitenant)" (the OAuth client uses the `/organizations/` endpoint — work/school accounts only, no personal MSAs), with a client secret. Both `session#microsoft` (sign-in) and `EmailAccountsController#create` (connect) read these, so they're required for either flow. ⚠️ **All Microsoft surfaces are disabled by default** (not production-ready) behind `Features.microsoft?` — set `ENABLE_MICROSOFT=1` to reveal the sign-in button + mailbox connect (legacy `ENABLE_MICROSOFT_MAILBOX` still honored); when off, the buttons are hidden and `session#microsoft` / the OAuth callback 404.
- `ENABLE_WORKFLOWS`, `ENABLE_EMAIL_BOARD`, `ENABLE_MICROSOFT` — **production-readiness feature gates** (`Features`, `app/models/features.rb`): features that are built but not yet production-ready ship OFF by default in every build; set to `1` to opt in. Orthogonal to the per-workspace `Entitlements` billing gates. Gate, respectively, the Workflow engine, the inbox "Board" (status kanban) layout, and all Microsoft 365 surfaces. Exceptions — `ENABLE_IMAP` and `ENABLE_TASKS` are **opt-OUT** gates that ship ON: IMAP mailbox connect and asks (the rethought Tasks module) are enabled by default; set `ENABLE_IMAP=0` / `ENABLE_TASKS=0` to hide them.
- `GMAIL_PUBSUB_TOPIC` — full Cloud Pub/Sub topic name (`projects/<project>/topics/<name>`) for Gmail real-time push; leave unset to use the minute poll.
- `GMAIL_PUBSUB_TOKEN` — shared secret appended to the push-subscription URL as `?token=<value>`; verified by `EmailWebhooksController` on every Google POST.
- `DISABLE_GMAIL_PUSH=1` — force-disables Gmail push even when the two vars above are set (useful to temporarily fall back to polling without changing the subscription).
