...
Customer-facing REST API for programmatic access to a workspace's own data. **Doorkeeper** provides OAuth 2.0 with the **client_credentials** grant only. Full reference: [`docs/api.md`](docs/api.md). Built in phases — **Phases 1-2 (live): emails, documents (incl. update/approve/reject/reclassify), contacts, tags (email-only), document types**; Phase 3 (workflows trigger, Scout) is planned.
...
The `website/` directory contains a **Docusaurus 3.10** static site (React 19, `preset-classic`) for the public landing page, docs, blog, pricing, about, and legal pages.

```
cd website && npm start          # Docusaurus dev server (add -p to avoid Rails :3000)
cd website && npm run build      # Production build to website/build/ (all locales)
```

Docs and blog are markdown in `website/docs/` and `website/blog/`; the landing/pricing/about/legal pages are React `.tsx` in `website/src/pages/`.

**i18n** (en · pt · es · fr): set up in `docusaurus.config.ts` (`i18n.locales`, `localeConfigs`, navbar `localeDropdown`). Translations live under `website/i18n/<locale>/` — theme/navbar/footer + `.tsx` page strings in `code.json`, and translated docs/blog markdown under `docusaurus-plugin-content-docs/current/` etc. `.tsx` strings are wrapped in `<Translate>`/`translate()`; after adding new ones run `npm run write-translations -- --locale <l>`, then translate the new `code.json` entries. ⚠️ The Edit tool can inject smart/curly quotes (`" "`) as JSON delimiters in `code.json` — validate (`node -e "require('./i18n/<l>/code.json')"`) or build after editing.
