...
- **Accounting — zip export, invoice chasing, and notifications (`ENABLE_ACCOUNTING=1`).** Export any completed reconciliation as a structured zip archive: the statement CSV, organised debit/credit sub-folders with matched invoices (preferring AI-processed PDFs), and an `index.csv` with 11 columns per transaction (NIF status, cross-reconciliation flag). Export runs in the background (`Reconciliations::ExportJob`) with live status updates and a download button on the show page. Request a missing invoice from the Resolve Panel: a pre-filled composer opens (standard or corrected-NIF variant) so you review and send — nothing is sent automatically; the transaction enters a "Invoice requested" state with Undo. Resetting a transaction now clears the request record. Three new notifications: reconciliation ready for review, CSV parse failure, and zip download ready.
- **Accounting — matching engine, workbench, and NIF check (`ENABLE_ACCOUNTING=1`).** Matching engine (`Reconciliations::Matcher`) scores each bank transaction against workspace documents using a heuristic (amount exactness, date proximity, name Jaccard similarity) and optionally disambiguates candidates with AI text analysis. Scores ≥ 0.85 auto-suggest a match; 0.5–0.85 surfaces the top candidate. A `MatchJob` runs after PDF/CSV parsing and broadcasts live progress via Turbo Streams. The workbench lets users confirm, reject, or exclude each transaction inline (desktop: hover-revealed ✓/✗ buttons; mobile: full-width actions on suggested cards). A lazy "Resolve" panel lets users manually search and attach any workspace document or mark a transaction as excluded with a reason. "Confirm all" bulk-confirms every top suggestion at once. A SummaryBar shows live matched/suggested/unmatched/excluded counts. PDF bank statements now parse via AI (`Ai::BankStatementParser`, provider-aware: native PDF for Anthropic/Gemini, page-rasterize for OpenAI/Mistral, chunked at 15 pages). Company VAT / NIF field added to Settings → General; documents with NIF mismatches are flagged with an amber indicator.
