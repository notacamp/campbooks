...
class NotificationLinkUrlRepairTest < ActiveSupport::TestCase
  setup do
  ...
  test "rebuilds Document links from the migrated notifiable_id" do
    n = notif(link_url: "/documents/189", notifiable_type: "Document", notifiable_id: @doc_uuid)
    run_migration!
    assert_equal "/documents/#{@doc_uuid}", n.reload.link_url
  end

  test "rebuilds scout AgentThread links from the migrated notifiable_id" do
    n = notif(link_url: "/scout/threads/42", notifiable_type: "AgentThread", notifiable_id: @thread_uuid)
    run_migration!
    assert_equal "/scout/threads/#{@thread_uuid}", n.reload.link_url
  end

  test "neutralises stale document links that carry no notifiable" do
    n = notif(link_url: "/documents/7") # "new document uploaded" activity: no notifiable
    run_migration!
    assert_equal "/documents", n.reload.link_url
  end

  test "neutralises stale email_messages links with no notifiable" do
    n = notif(link_url: "/email_messages/99")
    run_migration!
    assert_equal "/email_messages", n.reload.link_url
  end
  ...
  test "does not corrupt EmailThread links whose notifiable is an AgentThread" do
    n = notif(link_url: "/email_threads/55", notifiable_type: "AgentThread", notifiable_id: @agent_uuid)
    run_migration!
    assert_equal "/email_messages", n.reload.link_url
    refute_includes n.link_url, @agent_uuid
  end

  test "neutralises stale scout links with no notifiable" do
    n = notif(link_url: "/scout/threads/88")
    run_migration!
    assert_equal "/scout", n.reload.link_url
  end

  test "leaves already-migrated uuid links untouched" do
    healthy = "/documents/#{SecureRandom.uuid}"
    n = notif(link_url: healthy, notifiable_type: "Document", notifiable_id: @doc_uuid)
    run_migration!
    assert_equal healthy, n.reload.link_url
  end

  test "leaves non-deep-link urls (index, query-string) untouched" do
    ...
    run_migration!
    assert_equal "/documents?review_status=pending", a.reload.link_url
    assert_equal "/email_messages?inbox_settings=accounts", b.reload.link_url
    assert_equal "/contacts", c.reload.link_url
  end
  ...
  test "is idempotent -- a second run changes nothing" do
    ...
    run_migration!
    assert_equal first, [ doc, up, mention ].map { |n| n.reload.link_url }
    assert_equal "/documents/#{@doc_uuid}", doc.reload.link_url
  end
end
