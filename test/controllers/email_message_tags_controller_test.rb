...
class EmailMessageTagsControllerTest < ActionDispatch::IntegrationTest
  setup do
  ...
  test "adding an external tag calls the provider and creates the join" do
    ...
    assert_difference -> { message.reload.tags.count }, 1 do
    ...
    assert_requested :put, "#{zoho_base}/accounts/#{account.provider_account_id}/updatemessage"
  end

  # 6. Adding a local tag creates the join row with no HTTP.
  test "adding a local tag creates the join with no HTTP" do
    account = create_zoho_account
    message = create_message(account)
  ...
  test "removing an external tag calls the provider and removes the join" do
    account = create_zoho_account
    stub_zoho_token_refresh(refresh_token: account.refresh_token)
    message = create_message(account)
    tag = create_external_tag(account)
    ...
    post email_message_tags_path(message), params: { tag_id: tag.id }, as: :turbo_stream
    assert_equal 1, message.reload.tags.count
    ...
    assert_requested :put, "#{zoho_base}/accounts/#{account.provider_account_id}/updatemessage", times: 2
  end
end
