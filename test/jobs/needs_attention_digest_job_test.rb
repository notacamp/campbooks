...
class NeedsAttentionDigestJobTest < ActiveSupport::TestCase
  ...
  test "fans out a per-user job only for opted-in users with a workspace" do
    ...
    NeedsAttentionDigestJob.perform_now

    fanned_out = enqueued_jobs.select { |job| job[:job] == NeedsAttentionDigestMailJob }
