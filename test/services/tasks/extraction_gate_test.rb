...
module Tasks
  class ExtractionGateTest < ActiveSupport::TestCase
    setup do
      @workspace = Workspace.create!(name: "Gate WS")
    ...
    test "allows a human ask in English" do
      email = build_email(subject: "Contract", body: "<p>Please review the attached contract.</p>")

      assert ExtractionGate.email_allows?(email)
    end

    test "allows a human ask in Portuguese (the old English-only list skipped these)" do
      email = build_email(
    ...
    test "skips FYI mail with no action language" do
      email = build_email(subject: "Extrato Combinado", body: "<p>O seu extrato mensal.</p>")

      refute ExtractionGate.email_allows?(email)
    end

    test "vetoes machine categories however imperative the wording" do
      email = build_email(
      ...
      refute ExtractionGate.email_allows?(email)
      assert ExtractionGate.vetoed?(email)
    end

    test "vetoes no-reply senders even in a human-looking category" do
      email = build_email(
    ...
    test "vetoes outbound mail even when From carries a display name" do
      email = build_email(
        subject: "Please review", body: "<p>Please review the draft.</p>",
        from: "Mailbox Owner <mailbox@example.com>"
      )

      assert email.outbound?
      refute ExtractionGate.email_allows?(email)
    end

    test "vetoes Precedence: junk" do
      email = build_email(body: "<p>Please confirm your win!</p>", precedence: "junk")

      refute ExtractionGate.email_allows?(email)
    end

    test "screens replies on what the sender just wrote, not the quoted history" do
      email = build_email(
        subject: "Re: contract",
