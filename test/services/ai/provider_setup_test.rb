...
module Ai
  class ProviderSetupTest < ActiveSupport::TestCase
    ...
    test "apply_managed seeds keyless managed adapters for both text and documents" do
      with_env(TEXT_KEY => "k") do
        @setup.apply_managed

        text = @ws.ai_adapters.find_by(managed: true, provider: Ai::Platform::MANAGED_TEXT_PROVIDER)
        assert text, "expected a managed text adapter"
    ...
    test "apply_managed skips everything when the platform key is missing" do
