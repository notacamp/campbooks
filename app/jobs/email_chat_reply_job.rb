class EmailChatReplyJob < ApplicationJob
  ...
  def perform(agent_message_id)
    ...
    # Scout only replies when explicitly tagged. Checked before claiming so a
    # plain comment never gets stranded in :processing.
    return unless message.mentions_scout?
    ...
    # Establish the acting identity so the tool loop is scoped to this user's
    # permitted accounts (no session cookie exists in a background job).
    ...
    if result.blank? || result[:reply].blank?
      ...
      # a calm error in the thread.
      message.update_column(:reply_status, :failed)
      broadcast_failure(email_thread)
      return
    end
    ...
    if failures.any?
      prefix = I18n.t("jobs.email_chat_reply.auto_action_failures_prefix",
    ...
    # Scout's reply is new activity — let everyone following the thread know
    # (quiet bell entry, in-app only; no email for AI replies).
  ...
  # error (mirrors the global Scout chat). Broadcast reaches every viewer of the
  # thread who is subscribed.
  def broadcast_failure(email_thread)
    return unless email_thread

    Turbo::StreamsChannel.broadcast_remove_to(email_thread, target: "scout_typing")
    Turbo::StreamsChannel.broadcast_append_to(
      email_thread,
      target: "comments_list",
      partial: "email_comments/error"
    )
  rescue => e
    Rails.logger.warn("[EmailChatReplyJob] error broadcast failed: #{e.message}")
  end
  ...
  def execute_auto_actions(email_message, auto_actions)
    auto_actions.filter_map do |action|
      ...
      unless EmailActions.auto_safe?(tool)
        Rails.logger.warn("[EmailChatReplyJob] Blocked auto-execution of unsafe action '#{tool}' — must be confirmed by the user")
        next
      end
  ...
  def broadcast_reply(reply, auto_results)
    ...
    auto_results.each do |r|
      ...
      when "archive", "trash"
        ...
        archived_msg = I18n.t("jobs.email_chat_reply.conversation_archived",
                               locale: agent_thread.user.locale.presence || I18n.default_locale)
