...
export default class extends Controller {
  ...
  addStage() {
    ...
    this._stageIndex++
    // The template's position field ships blank; without this the new stage
    // submits position="" and fails the presence validation.
  ...
  dragOver(event) {
    ...
    row.classList.add("ring-2", "ring-accent-400", "ring-inset")
  }

  dragLeave(event) {
    const row = event.currentTarget.closest("[data-pipeline-builder-target='stageRow']")
    if (!row || row.contains(event.relatedTarget)) return // moved onto a child input, not leaving
    this._clearRowHighlight(row)
  }
  ...
  drop(event) {
    ...
    this._clearRowHighlight(dropRow)
  ...
  dragEnd() {
    ...
    this.stageRowTargets.forEach((r) => this._clearRowHighlight(r))
    this._draggedRow = null
  }

  _clearRowHighlight(row) {
    row.classList.remove("ring-2", "ring-accent-400", "ring-inset")
  }

  _renumberPositions() {
    const rows = this.stagesTarget.querySelectorAll(
      "[data-pipeline-builder-target='stageRow']:not(.hidden)"
    )
    rows.forEach((row, i) => {
