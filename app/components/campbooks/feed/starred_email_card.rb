...
module Campbooks
  module Feed
    ...
    class StarredEmailCard < Campbooks::Feed::Base
      ...
      # Scout's read leads when present, capped at three Ember lines with a "Read
      # more", with the full message one collapsed tap below; otherwise the email
      # excerpt is the body.
      def body
        if (msg = scout_message).present?
          render Campbooks::ScoutNote.new(message: msg, compact: true, lines: 3, class: "mt-3")
