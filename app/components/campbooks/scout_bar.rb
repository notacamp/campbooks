  ...
  class ScoutBar < Campbooks::Base
    ...
    def desktop_bar
      div(class: "pointer-events-none fixed inset-x-0 bottom-4 z-30 hidden px-4 lg:left-20 lg:flex lg:justify-center") do
        bar_control(
          class_names(
            "pointer-events-auto flex w-full items-center gap-2.5 rounded-2xl border border-border bg-card/80 p-2 pl-3.5 text-left shadow-lg backdrop-blur-xl transition hover:bg-card",
            @desktop_max_width
          ),
          coach: @coach_anchor
        ) do
    ...
    def mobile_bar
      div(class: "pointer-events-none fixed inset-x-4 bottom-[calc(4rem+env(safe-area-inset-bottom)+0.75rem)] z-30 flex justify-center lg:hidden") do
        bar_control(
          "pointer-events-auto flex h-[46px] w-full items-center gap-2.5 rounded-2xl border border-border bg-card/80 pl-3.5 pr-2 text-left shadow-lg backdrop-blur-xl transition hover:bg-card"
        ) do
          spark
