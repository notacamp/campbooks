...
module Campbooks
  ...
  #   * compact — one Ember spark + a bold "Scout" + the read on a single clamped
  #     line, for the dense home feed where a block on every card is too much. Still
  #     unmistakably Scout (the Meaning Rule), at a fraction of the height.
  ...
  # @param compact [Boolean] render the one-line shape
  class ScoutNote < Campbooks::Base
    ...
    def initialize(message:, time: nil, compact: false, **attrs)
      @message = message
      @time = time
      @compact = compact
      @attrs = attrs
    ...
    def compact_template
      div(class: class_names("flex items-start gap-1.5 text-[13px] leading-relaxed", @attrs.delete(:class)), **@attrs) do
        ...
        # Scout's read can run long; clamp it to ~10 lines with a "Read more"
        # toggle rather than cutting it at one line. The Scout label rides inside
        # the clamped text so it stays unmistakably Scout (the Meaning Rule).
        render Campbooks::ClampText.new(lines: 10, class: "min-w-0 flex-1 text-foreground/80") do
