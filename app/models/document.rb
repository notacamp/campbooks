class Document < ApplicationRecord
  ...
  # First-page render for the Files grid view, generated by
  # Documents::ThumbnailGenerator (ImageMagick/Ghostscript) right after create.
  # Attached to the Document itself — NOT an Active Storage variant/preview:
  # those attach to bigint-keyed VariantRecord/Blob rows, which this schema's
  # uuid `active_storage_attachments.record_id` cannot reference.
  ...
  # True when the original file is a kind the thumbnail pipeline can render
  # (a PDF page or an image). Everything else keeps the type-icon tile.
  def thumbnailable?
    pdf? || image?
  end
...
") + para
  ...
  # Grid-view thumbnails render on the worker, off the request path. Best-effort:
  # a failed enqueue (queue unreachable) must never fail the document create —
  # the tile just keeps its icon until the backfill sweep catches it.
  def enqueue_thumbnail_generation
    Documents::GenerateThumbnailJob.perform_later(id) if thumbnailable?
  rescue StandardError => e
    Rails.logger.warn("[Document] thumbnail enqueue failed for #{id}: #{e.message}")
  end
