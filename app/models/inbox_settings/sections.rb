...
module InboxSettings
  ...
  module Sections
    ...
    # Minimal stroke-icon `<path>` markup keyed by `:icon`. Kept as data so both
    # the Phlex modal and the ERB settings page can wrap it in an <svg>.
    ICON_PATHS = {
    ...
    # i18n key for a section's nav label. Shared across the modal + settings page.
    def label_key(key)
      "components.inbox_settings_modal.nav.#{key}"
    end

    # A section's nav icon as an inline <svg> string.
    def icon_svg(name, css: "w-4 h-4")
      %(<svg class="#{css}" fill="none" stroke="currentColor" viewBox="0 0 24 24">#{ICON_PATHS[name]}</svg>)
    end
  end
end
