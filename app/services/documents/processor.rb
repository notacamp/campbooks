module Documents
  class Processor
    ...
    def call
      ...
      if result[:error]
        Rails.logger.warn("[Documents::Processor] AI analysis failed for document #{@document.id}: #{result[:error]}")
      ...
      Rails.logger.error("[Documents::Processor] Error processing document #{@document.id}: #{e.message}")
