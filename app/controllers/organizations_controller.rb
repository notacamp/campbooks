class OrganizationsController < ApplicationController
  before_action :require_organizations_enabled
  before_action :require_authentication
  before_action :set_organization, only: %i[show update emails documents]

  def index
    @organizations = Current.workspace.organizations
      .includes(:people)
      .ordered
  ...
  def show
    ...
    @recent_documents = Document.by_organization(@organization)
  ...
  def documents
    @pagy, @documents = pagy(
      Document.by_organization(@organization)
        .includes(:classification)
        .order(created_at: :desc),
      items: 30
    )
