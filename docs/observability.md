...
rendered in the Prometheus text format at **`GET /metrics`**. They are **off by
default**: the endpoint and the custom metrics only activate when
`PROMETHEUS_MULTIPROC_DIR` is set (the standard multi-process layout) or
`CAMPBOOKS_METRICS_ENABLED=1`, so a build that doesn't opt in stays inert.
...
### Application — from `lib/campbooks/metrics.rb`
...
process write there; the endpoint aggregates them at scrape time. The wiring
lives in `lib/campbooks/metrics.rb` (activated by
...
Without `PROMETHEUS_MULTIPROC_DIR` (single-process self-host or dev) set
`CAMPBOOKS_METRICS_ENABLED=1` to use the default in-memory store; then only
`campbooks-web:3000` need be scraped.
