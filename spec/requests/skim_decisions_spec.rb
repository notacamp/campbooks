...
# The controller wiring: each Skim decision endpoint logs a SkimDecision so the
# learning loop (Emails::SkimActionMemory) has something to learn from. The
# signature derivation itself is covered in skim_decision_recorder_spec.
RSpec.describe "Skim decisions", type: :request do
  ...
  it "logs an archive decision when a stack is archived" do
    ...
    end.to change { SkimDecision.where(action: "archive", user: user).count }.by(1)
