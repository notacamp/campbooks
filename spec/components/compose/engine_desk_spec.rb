...
# The Desk-layout Engine anatomy (compose from intent). The intent input, chip
# envelope, persistent saved status, and the AI-gated tone footer.
RSpec.describe Campbooks::Compose::Engine, type: :component do
  def render_desk(ai: true, **overrides)
    allow_any_instance_of(ApplicationController).to receive(:ai_provider_available?).and_return(ai)
    args = {
      shell: :desk, desk: true, persistent_status: true, mode: :new_message,
  ...
  it "marks an inferred recipient and subject" do
    html = render_desk(to: "Sofia Martins <sofia@example.com>", to_inferred: true,
  ...
  it "does not mark fields inferred when they were not" do
    html = render_desk(to_inferred: false, subject_inferred: false)
