...
RSpec.describe "UUID foreign keys" do
  ...
  it "every belongs_to onto a uuid table has a uuid foreign-key column" do
  ...
  it "polymorphic *_id columns that can reference uuid tables are uuid" do
    polymorphic = [
    ...
    expect(offenders).to be_empty, "Polymorphic id columns must be uuid:\n  #{offenders.join("\n  ")}"
  end
