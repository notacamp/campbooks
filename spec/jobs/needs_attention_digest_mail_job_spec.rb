...
RSpec.describe NeedsAttentionDigestMailJob, type: :job do
  ...
  let(:ws) { Workspace.create!(name: "Attention Digest WS") }
  let(:user) { ws.users.create!(name: "Dana", email_address: "dana-attn@example.com", password: "changeme123") }
  let(:account) do
    acct = EmailAccount.create!(workspace: ws, email_address: "dana-attn@example.com", refresh_token: "tok")
    EmailAccountUser.create!(user: user, email_account: acct, can_read: true)
    acct
  end

  before { account } # ensure account + user link created

  # ── Follow-ups section ────────────────────────────────────────────────────────

  it "sends when user has due waiting threads" do
  ...
  it "sends when user has overdue reminders" do
  ...
  it "does not include reminders due farther than 1 day ahead" do
    email_msg = EmailMessage.create!(
  ...
  it "sends when user has overdue tasks" do
  ...
  it "sends when user has high-priority tasks with no due date" do
  ...
  it "does not include low-priority tasks without a due date" do
  ...
  it "does nothing when user opted out" do
  ...
  it "does nothing when all sections are empty" do
    expect {
      described_class.perform_now(user.id)
    }.not_to have_enqueued_job(ActionMailer::MailDeliveryJob)
  end
