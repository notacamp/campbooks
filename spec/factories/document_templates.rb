    ...
    trait :ai_completed do
      ...
      html_content do
        "<!DOCTYPE html><html><head><style>body{font-family:sans-serif}</style></head>" \
          "<body><h1>Hello {{ recipient_name }}</h1><p>Amount: {{ amount }}</p></body></html>"
      end
      variables_schema do
        [
          { "key" => "recipient_name", "label" => "Recipient Name", "type" => "string", "required" => true, "default" => "" },
          { "key" => "amount", "label" => "Amount", "type" => "number", "required" => false, "default" => "0" }
        ]
      end
