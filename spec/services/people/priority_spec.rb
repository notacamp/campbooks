...
RSpec.describe People::Priority do
  ...
  def standing(needs_you: false, kind: :none, overdue_days: 0)
    People::Standing::Result.new(text: "x", needs_you: needs_you, thread_id: nil, overdue_days: overdue_days, kind: kind)
  end
  ...
  def facts(**overrides)
    described_class::Facts.new(
      **{
        ...
        action_prompt: false, important: false, follow_up_due: false, last_activity: now - 1.day
      }.merge(overrides)
    )
  end
  ...
  describe "Need you" do
    it "a genuine ask from a regular outranks a stranger's older one" do
      client = regular(standing: standing(needs_you: true, kind: :you_owe, overdue_days: 2))
      stranger = one_off(standing: standing(needs_you: true, kind: :you_owe, overdue_days: 14))
      expect(score(client)).to be > score(stranger)
    end

    it "between equals, the one waiting longer comes first" do
      long = regular(standing: standing(needs_you: true, kind: :you_owe, overdue_days: 9))
      short = regular(standing: standing(needs_you: true, kind: :you_owe, overdue_days: 2))
      expect(score(long)).to be > score(short)
    end

    it "a reply you owe outranks a nudge you could send" do
      owe = regular(standing: standing(needs_you: true, kind: :you_owe, overdue_days: 5))
      nudge = regular(standing: standing(needs_you: true, kind: :nudge, overdue_days: 5))
      expect(score(owe)).to be > score(nudge)
    end

    it "a Scout prompt, an important read and a due follow-up each add a bonus" do
      base = regular(standing: standing(needs_you: true, kind: :you_owe, overdue_days: 3))
    ...
    it "flags needs_you on the score" do
      expect(described_class.score(regular(standing: standing(needs_you: true, kind: :you_owe)), now: now).needs_you).to be true
  ...
  describe ".facts_for" do
    it "derives the relationship facts from loaded threads, contacts and the latest inbound" do
      ...
      f = described_class.facts_for(standing: standing, threads: threads, contacts: contacts, latest_inbound: latest,
                                    relationship_type: "client", last_activity: now - 2.days, now: now)

      expect(f.two_way_threads).to eq(1)
      expect(f.outbound_threads).to eq(2)
      ...
      expect(f).to have_attributes(starred: true, allowed: true, classified: true, relationship_type: "client",
                                   action_prompt: true, important: true, follow_up_due: true)
    end

    it "reads an important category as important, and blank facts for a bare one-off" do
      latest = build_stubbed(:email_message, category: "important", ai_priority: :medium)
      f = described_class.facts_for(standing: standing, threads: [], contacts: [ build_stubbed(:contact, email_count: 1) ],
                                    latest_inbound: latest, relationship_type: "", last_activity: nil, now: now)
      expect(f).to have_attributes(two_way_threads: 0, outbound_threads: 0, email_count: 1, starred: false,
                                   allowed: false, classified: false, relationship_type: nil, action_prompt: false,
                                   important: true, follow_up_due: false, last_activity: nil)
      ...
      expect(bare).to have_attributes(email_count: 0, important: false)
    end
  end

  describe ".merge_facts" do
    it "folds an organization's people into the strongest, liveliest facts" do
      a = regular(last_activity: now - 5.days, relationship_type: "vendor")
      b = one_off(starred: true, last_activity: now - 1.day, relationship_type: "client")
      org_standing = standing(needs_you: true, kind: :you_owe, overdue_days: 4)

      m = described_class.merge_facts([ a, b ], standing: org_standing)
      expect(m.two_way_threads).to eq(3)
      expect(m.email_count).to eq(21)
      expect(m.starred).to be true
      expect(m.relationship_type).to eq("client")
      expect(m.last_activity).to eq(now - 1.day)
      expect(m.standing).to eq(org_standing)
      expect(described_class.merge_facts([], standing: org_standing)).to be_nil
    end
  end
end
