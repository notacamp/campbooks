...
namespace :tasks do
  desc "Re-enqueue task extraction for recent mail the gate admits (DAYS=7 back, " \
       "DRY_RUN=true by default). Backfills mail ingested before asks shipped — or " \
       "whose extraction was lost to a transient provider error. The analyzer now " \
       "stages asks per email, so this is the only path that still runs the standalone " \
       "extractor. The job re-checks the feature flag and gate; the builder is idempotent."
